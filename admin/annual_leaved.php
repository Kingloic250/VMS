...
if ($_SERVER['REQUEST_METHOD'] == 'POST') {
    ...
    if (!is_numeric($year)) {
        ...
        $select = mysqli_query($connect,"SELECT *FROM leave_citizen JOIN isibo ON (leave_citizen.i_id=isibo.i_id) JOIN village ON (isibo.v_id=village.v_id)
                 JOIN cell_table ON (cell_table.cell_id=village.cell_id) JOIN sector ON (cell_table.s_id=sector.s_id)
                  JOIN district ON (district.d_id=sector.d_id) JOIN user_table ON (district.d_id=user_table.d_id)
                   WHERE reg_year = '$year' AND user_id = '$user_id'");

        if (mysqli_num_rows($select) > 0) {
            ?>
            <title>VMS - leaved Citizen</title>
			     ...
			     <div class="row">
				    <div class="col-md-12">
					   ...
					   <div class="table-title">
					     <div class="row">
                         <a href="annual_leave.php"><i class="fas fa-arrow-left" style="margin-top:10px;color:#eee;background:rgb(60, 60, 60);"></i></a>
...
<?php
                                if (isset($errormsg)) {
                                    ...
                                    <a href="annual_leave.php"><button class="btn btn-secondary" style="margin-left:450px;">Back</button></a>
                                    <?php
                                }
                                ?>
</div>

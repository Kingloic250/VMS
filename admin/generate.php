...
if ($_SERVER['REQUEST_METHOD'] == 'POST') {
    ...
    elseif (!is_numeric($year)) {
        ...
        $select = mysqli_query($connect,"SELECT *FROM akagoroba JOIN village ON (akagoroba.v_id=village.v_id)
                 JOIN cell_table ON (cell_table.cell_id=village.cell_id) JOIN sector ON (cell_table.s_id=sector.s_id)
                  JOIN district ON (district.d_id=sector.d_id) JOIN user_table ON (district.d_id=user_table.d_id)
                   WHERE a_month = '$month' AND a_year = '$year' AND user_id = '$user_id'");

        if (mysqli_num_rows($select) > 0) {
            ?>
            <title>VMS - Akagoroba</title>
			     ...
			     <div class="row">
				    <div class="col-md-12">
					   ...
					   <div class="table-title">
					     <div class="row">
                         <a href="akagoroba.php"><i class="fas fa-arrow-left" style="margin-top:10px;color:#eee;background:rgb(60, 60, 60);"></i></a>
            ...
            <table class="table table-hover">
						  ...
						  <tbody>
							...
							?>
							 ...
							 <th><?php echo $get['a_month']."-".$get['a_year'] ?></th>
...
<?php
                                if (isset($errormsg)) {
                                    ...
                                    <a href="akagoroba.php"><button class="btn btn-secondary" style="margin-left:450px;">Back</button></a>
                                    <?php
                                }
                                ?>
</div>

...
if (isset($_POST['update'])) {
	...
	elseif (empty($child)) {
		$errormsg = '<i class="fas fa-exclamation-circle"></i> Number of children in school required';
	}
...
$records = mysqli_query($connect,"SELECT *FROM citizen JOIN isibo ON (citizen.i_id=isibo.i_id) JOIN village ON (isibo.v_id=village.v_id) JOIN cell_table ON (village.cell_id=cell_table.cell_id) JOIN exective ON (cell_table.cell_id=exective.cell_id) WHERE e_id = '$user_id'");
...
$retreive = mysqli_query($connect,"SELECT *FROM citizen JOIN isibo ON (citizen.i_id=isibo.i_id) JOIN village ON (isibo.v_id=village.v_id) JOIN cell_table ON (village.cell_id=cell_table.cell_id) JOIN exective ON (cell_table.cell_id=exective.cell_id) WHERE e_id = '$user_id' LIMIT $start,$rows_per_page");

$check = mysqli_query($connect,"SELECT *FROM citizen JOIN isibo ON (citizen.i_id=isibo.i_id) JOIN village ON (isibo.v_id=village.v_id) JOIN cell_table ON (village.cell_id=cell_table.cell_id) JOIN exective ON (cell_table.cell_id=exective.cell_id) WHERE e_id = '$user_id'");
?>
					   ...
					   <div class="table-title">
					     <div class="row">
						     <div class="col-sm-6 p-0 flex justify-content-lg-start justify-content-center">
							    <h2 class="ml-lg-2">Manage the Citizens(Abaturage) in a cell (<?php echo mysqli_num_rows($check)  ?>)</h2>
							 </div>
					     </div>
					   </div>
					   <?php

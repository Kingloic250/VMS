...
$records = mysqli_query($connect,"SELECT * FROM leave_citizen WHERE i_id = '$i_id'");
...
$retreive = mysqli_query($connect,"SELECT *FROM leave_citizen WHERE i_id = '$i_id' LIMIT $start,$rows_per_page");

$sql = mysqli_query($connect,"SELECT *FROM isibo JOIN village ON (village.v_id=isibo.v_id) JOIN cell_table ON (village.cell_id=cell_table.cell_id) WHERE i_id = '$i_id'");
$fetch = mysqli_fetch_assoc($sql);

$check = mysqli_query($connect,"SELECT *FROM leave_citizen WHERE i_id = '$i_id'");
?>

<title>VMS - Manage Leaving Citizens</title>
				    ...
				    <div class="col-md-12">
					   ...
					   <div class="table-title">
					     <div class="row">
						     ...
						     <div class="col-sm-7 p-0 flex justify-content-lg-start justify-content-center">
							    <h2 class="ml-lg-3">Manage Leaving Citizens(Abaturage) in Sub village (<?php echo mysqli_num_rows($check) ?>)</h2>
							 </div>
					     </div>
					   </div>
					   <?php
					   ...
					   <table class="table table-striped table-hover">
					      <thead>
							 ...
							 <th class="text-body">Leaving Date</th>
...
<div class="box">
  ...
  <div class="modal-dialog" role="document">
    <div class="modal-content">
      <div class="modal-header">
        <h5 class="modal-title">Add a leave_citizen</h5>
        <button type="button" class="close" data-dismiss="modal" aria-label="Close">
          <span aria-hidden="true">&times;</span>
        </button>
      </div>
  ...
  <div class="modal-dialog" role="document">
    <div class="modal-content">
      <div class="modal-header">
        <h5 class="modal-title">Edit leave_citizen</h5>
        <button type="button" class="close" data-dismiss="modal" aria-label="Close">
          <span aria-hidden="true">&times;</span>
        </button>
      </div>
...
<script>
	...
	//passing leave_citizen id in deleting modal using ajax js
	...
	$(".confirm-delete").on('click', function (e) {
		let id = $(this).attr('data-id');
		console.log(id);
		location.href = "leave_delete.php?id= "+id;
	});

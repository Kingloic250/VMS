...
if (isset($_POST['submit'])) {
	...
	elseif (!is_numeric($year)) {
        ...
        $select = mysqli_query($connect,"SELECT *FROM umuganda WHERE u_month = '$month' AND u_year = '$year'");
        if (mysqli_num_rows($select) == 0) {
            $res = mysqli_query($connect,"INSERT INTO umuganda VALUES ('$u_id','$v_id','$place','$text','$day','$month','$year')");
            if ($res == true) {
                // automaticaly insert
            }
            else {
            ...
            $errormsg = '<i class="fas fa-exclamation-circle"></i> Please check date, no two activities in same month and year';
        }
    }
}
...
if (isset($_POST['update'])) {
	...
	elseif (!is_numeric($year)) {
		$errormsgs = '<i class="fas fa-exclamation-circle"></i> year required';
            ...
            $update = mysqli_query($connect,"UPDATE umuganda SET place = '$place',descriptions = '$text',u_day = '$day',u_month= '$month',u_year = '$year' WHERE u_id = '$id'");
            if ($update == true) {
				// automaticaly update
			}
			else {
...
$records = mysqli_query($connect,"SELECT *FROM umuganda JOIN village ON (umuganda.v_id=village.v_id) JOIN mudugudu ON (mudugudu.v_id=village.v_id) WHERE m_id = '$m_id'");
...
$retreive = mysqli_query($connect,"SELECT *FROM umuganda JOIN village ON (umuganda.v_id=village.v_id) JOIN mudugudu ON (mudugudu.v_id=village.v_id) WHERE m_id = '$m_id' LIMIT $start,$rows_per_page");

$check = mysqli_query($connect,"SELECT *FROM umuganda JOIN village ON (umuganda.v_id=village.v_id) JOIN mudugudu ON (mudugudu.v_id=village.v_id) WHERE m_id = '$m_id'");

?>
<!------main-content-start-----------> 
<title>VMS - Umuganda</title>
				    ...
				    <div class="col-md-12">
					   ...
					   <div class="table-title">
					     <div class="row">
						     <div class="col-sm-6 p-0 flex justify-content-lg-start justify-content-center">
							    <h2 class="ml-lg-2">Manage the Activities (Imiganda) (<?php echo mysqli_num_rows($check) ?>)</h2>
							 </div>
					   ...
					   <table class="table table-hover">
					      <thead>
                             ...
                             <th class="text-body">Description of Activity</th>
						  ...
						  <tbody>
							...
							?>
							 ...
							 <th><?php echo $get['descriptions'] ?></th>
							 <th><?php echo $get['u_day'] ."-".$get['u_month']."-".$get['u_year'] ?></th>
							 <th>
							   <button id="<?php echo $get['u_id']; ?>" type="button" data-toggle="modal" data-target="#editEmployeeModal" class="edit">	
							    <i class="fas fa-edit" data-toggle="tooltip" title="Edit"></i>
							   </button>
							   <button data-id="<?php echo $get['u_id']; ?>" type="button" data-toggle="modal" data-target="#deleteEmployeeModal" class="delete">
  ...
  <div class="modal-dialog" role="document">
    <div class="modal-content">
      <div class="modal-header">
        <h5 class="modal-title">Delete this activity</h5>
...
<div class="box">
  ...
  <div class="modal-dialog" role="document">
    <div class="modal-content">
      <div class="modal-header">
        <h5 class="modal-title">Register Umuganda</h5>
        <button type="button" class="close" data-dismiss="modal" aria-label="Close">
          <span aria-hidden="true">&times;</span>
        </button>
      </div>
  ...
  <div class="modal-dialog" role="document">
    <div class="modal-content">
      ...
      <div class="modal-body">
			...
			<div class="form-group">
				<input type="hidden" name="u_id" id="u_id" class="form-control" readonly>
			</div>
		</div>
      <div class="modal-footer">
        <button type="button" class="btn btn-secondary" data-dismiss="modal">Cancel</button>
...
<script>
	...
	$(".confirm-delete").on('click', function (e) {
		let id = $(this).attr('data-id');
		console.log(id);
		location.href = "deleteumug.php?id= "+id;
	});
	...
	$(document).ready(function() {
        ...
        $(document).on('click', '.edit', function(){  
           ...
           $.ajax({  
                url:"get.php",  
                method:"POST",  
                data:{fetch: 1,u_id:u_id},  
                dataType:"json",  
                success:function(data){  
                     $('#editEmployeeModal #place').val(data.place); 
					 $('#editEmployeeModal #v_id').val(data.village);
					 $('#editEmployeeModal #text').val(data.descriptions);
					 $('#editEmployeeModal #day').val(data.u_day); 
					 $('#editEmployeeModal #month').val(data.u_month); 
					 $('#editEmployeeModal #year').val(data.u_year);  
                     $('#editEmployeeModal #u_id').val(data.u_id);  
                }  
            });  
        });

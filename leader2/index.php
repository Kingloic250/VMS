...
if (isset($_POST['btn'])){
    ...
    if (empty($phone)){
        $errors = "<i class='fas fa-exclamation-circle'></i> Phone number required";
    }
    elseif (empty($password)){
        $errors = "<i class='fas fa-exclamation-circle'></i> Password required";
    }
    else{
        $select = mysqli_query($connect,"SELECT *FROM mutwarasibo WHERE phones = '$phone'");
        if ($select -> num_rows == 1){
            $fetch = $select -> fetch_assoc();
            if ($fetch['password'] == $password){
                $_SESSION['i_id'] = $fetch['i_id'];
                $_SESSION['mut_id'] = $fetch['mut_id'];
            ...
            $errors = "<i class='fas fa-exclamation-circle'></i> Invalid phone number";
        }
    }
}
...
<div class="img">
    <img src="../img/vms.png" alt="" style="width: 100px;height: 100px;margin-left: 20px;margin-top: 24px;">
            ...
            <?php endif ?>
                                <h3 style="font-family: 'poppins',sans-serif;" class="text-light">SIGN IN  VMS  LEADER 2 (MUTWARASIBO)</h3>
                                <div class="form-floating mb-3 mt-3" style="margin-top:40px;">
                                    <input type="text" style="color:white;font-family: 'poppins',sans-serif;border-radius:20px;background: transparent;" name="phone" class="form-control" placeholder="phone">
                                    <label for="uname" style="color:white;font-family: 'poppins',sans-serif;">Phone Number</label>
                                </div>
                                <div class="form-floating mb-3 mt-3">
                                    <input type="password" style="color:white;font-family: 'poppins',sans-serif;border-radius:20px;background: transparent;" name="password" class="form-control" placeholder="password">

...
if (!empty($_SESSION['mut_id'])) {

 	if (isset($_SESSION['last_timestamp']) && (time() - $_SESSION['last_timestamp']) > $inactive_time) {
		session_unset();
		session_destroy();
        echo "<script>alert('You have been signed out due to inactive session for 15 minutes.');window.location.href='../';</script>";
	}
	else {
	 ...
	 <div id="sidebar">
		...
		<ul class="list-unstyled component m-0">
		  ...
		  <ul class="collapse list-unstyled menu" id="homeSubmenu1">
		     <li><a href="register.php">Present / Abahari</a></li>
			 <li><a href="leave.php">Leaved / Abagiye</a></li>
		  </ul>
		  </li>
		  ...
		  <div class="top-navbar">
		     <div class="xd-topbar">
					     ...
					     <div class="xp-searchbar">
						     <form action="search_result.php" method="post">
							    <div class="input-group">
								  <input type="search" id="live_search" class="search-results" name="search"
								  placeholder="Search here..." autocomplete="off">
								  <i class="fas fa-search"></i>
								</div>
								<div id="search_result" name='search' class="results"></div>
							 </form>
						 </div>
					 </div>
				 ...
				 <div class="xp-breadcrumbbar text-center">
				    <h4 class="page-title">Leader 2(Mutwarasibo) Dashboard</h4>
					<ol class="breadcrumb">
...
<div class="modal fade show" tabindex="-1" id="signModal" role="dialog">
  <div class="modal-dialog" role="document">
    <div class="modal-content">
      ...
      <div class="modal-body">
        <p>Signing out due to inactive account for 15 min</p>
      </div>
      <div class="modal-footer">
        <a href="/vms/"><button type="button" class="btn btn-secondary">Ok</button></a>
      </div>
    </div>
  </div>
</div>
...
<div class="modal fade" tabindex="-1" id="signEmployeeModal" role="dialog">
  <div class="modal-dialog" role="document">
    <div class="modal-content">
      <div class="modal-header">
        <h5 class="modal-title">Signing Out Mutwarasibo</h5>
        <button type="button" class="close" data-dismiss="modal" aria-label="Close">
          <span aria-hidden="true">&times;</span>
        </button>
      </div>
      ...
      <div class="modal-footer">
        ...
        <a href="logout.php?user_id=<?php echo md5($_SESSION['mut_id']); ?>"><button type="button" class="btn btn-danger">Sign Out</button></a>
      </div>
    </div>
  </div>
</div>
...
$select = mysqli_query($connect,"SELECT *FROM isibo JOIN mutwarasibo ON (isibo.i_id=mutwarasibo.i_id) JOIN village ON (isibo.v_id=village.v_id) WHERE mut_id = '$mut_id'");
  ...
  <div class="modal-dialog" role="document">
    <div class="modal-content">
      ...
      <div class="modal-body">
		...
		<h6 class="title">Title: Chief, of <?php echo $fetch['i_name'] ?></h6> 
		<h6>Village of: <?php echo $fetch['village'] ?></h6>
		<h6>Phone Number: <?php echo '0'.strtolower($fetch['phones']) ?></h6>
      </div>
      <div class="modal-footer">
        <button type="button" class="btn btn-secondary" data-dismiss="modal">Cancel</button>
      </div>
